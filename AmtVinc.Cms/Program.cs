...
using AmtVinc.Cms.Localization;
...
// Hizmetler (liste + detay). Slug catch-all'dan ÖNCE eşleşir.
app.MapControllerRoute(
    name: "services",
    pattern: "{culture:regex(^[a-zA-Z]{{2}}$)}/hizmetler",
    defaults: new { controller = "Services", action = "Index" });
app.MapControllerRoute(
    name: "serviceDetail",
    pattern: "{culture:regex(^[a-zA-Z]{{2}}$)}/hizmet/{slug}",
    defaults: new { controller = "Services", action = "Detail" });

// Makine filosu (liste + detay).
app.MapControllerRoute(
    name: "machines",
    pattern: "{culture:regex(^[a-zA-Z]{{2}}$)}/makineler",
    defaults: new { controller = "Machines", action = "Index" });
app.MapControllerRoute(
    name: "machineDetail",
    pattern: "{culture:regex(^[a-zA-Z]{{2}}$)}/makine/{slug}",
    defaults: new { controller = "Machines", action = "Detail" });
